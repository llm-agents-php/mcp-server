    ...
    LoopAwareInterface
{
    ...
    public function __construct(
        ...
        if (
            ...
            && ($this->inputStreamResource === STDIN && $this->outputStreamResource === STDOUT)
        ) {
        ...
        if (
            !\is_resource($this->outputStreamResource)
            || \get_resource_type($this->outputStreamResource) !== 'stream'
        ) {
    ...
    public function listen(): void
    {
        ...
        $this->stdin->on('data', function ($chunk): void {
            $this->buffer .= $chunk;
            $this->processBuffer();
        });

        $this->stdin->on('error', function (\Throwable $error): void {
            $this->logger->error('STDIN stream error.', ['error' => $error->getMessage()]);
            $this->emit(
                'error',
                [new TransportException("STDIN error: {$error->getMessage()}", 0, $error), self::CLIENT_ID],
            );
            $this->close();
        });
        ...
        $this->stdout->on('error', function (\Throwable $error): void {
            $this->logger->error('STDOUT stream error.', ['error' => $error->getMessage()]);
            $this->emit(
                'error',
                [new TransportException("STDOUT error: {$error->getMessage()}", 0, $error), self::CLIENT_ID],
            );
            $this->close();
        });
    ...
    public function sendMessage(Message $message, string $sessionId, array $context = []): PromiseInterface
    {
        ...
        if ($written) {
            ...
            $this->stdout->once('drain', function () use ($deferred): void {
