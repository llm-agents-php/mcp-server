...
use Mcp\Server\Defaults\MimeType;
...
final class ResourceResultFormatter
{
     ...
     */
    public static function format(mixed $readResult, string $uri, ?string $mimeType): array
    {
            ...
            if ($hasResourceContents || $hasEmbeddedResource) {
                ...
                foreach ($readResult as $item) {
                    if ($item instanceof ResourceContents) {
                        ...
                        $result = \array_merge($result, self::format($item, $uri, $mimeType));
                    }
                }
                return $result;
            }
        ...
        if (\is_string($readResult)) {
            $mimeType ??= MimeType::guess($readResult);
        ...
        if ($readResult instanceof \SplFileInfo && $readResult->isFile() && $readResult->isReadable()) {
            if ($mimeType && \str_contains(\strtolower($mimeType), 'text')) {
                $content = \file_get_contents($readResult->getPathname());
                if ($content === false) {
                    throw new \RuntimeException("Failed to read file '{$readResult->getPathname()}'");
                }

                return [TextResourceContents::make($uri, $mimeType, $content)];
            }
